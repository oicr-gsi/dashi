...
machines = {
    "HiSeq": {"name": ["D00353", "D00331", "D00355", "D00343"]},
    "MiSeq": {"name": ["M00146", "M00753"]},
    "NovaSeq": {"name": ["A00469"]},
}


machine_ids = list(
    itertools.chain.from_iterable([l["name"] for l in machines.values()])
)
...
def machines_to_dispaly(df: pandas.DataFrame) -> bool:
    right_model = all(
        df.loc[
            (df["level"] == "flow_cell") & (df["key"] == "sequencerName"),
            "value",
        ].isin(machine_ids)
    )

    completed = all(df.loc[df["key"] == "healthType", "value"] == "Completed")
...
rs = rs.groupby("run_alias").filter(machines_to_dispaly)
...
def update_run_alias_dropdown(machine_name):
    if machine_name is None:
        return [{"label": k, "value": k} for k in rs["run_alias"].unique()]
    else:
        ...
        return [{"label": k, "value": k} for k in clust["run_alias"].unique()]
...
def update_clusters_pf(json_str):
    ...
    clust = df.loc[
        (rs["level"] == "flow_cell") & (rs["key"] == "Clusters PF"), "value"
    ].astype("int")

    return {"data": [{"x": clust, "type": "histogram"}]}
...
def display_hist_click(clicked, json_str):
    ...
    run_alias = df.loc[
        (df["level"] == "flow_cell") & (df["key"] == "Clusters PF"), "run_alias"
    ]
